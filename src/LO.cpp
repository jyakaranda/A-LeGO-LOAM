...
public:
      ...
      if (!seg_cloud_buf_.empty() && !seg_info_buf_.empty() && !outlier_buf_.empty())
      {
        ...
        for (int i = 0; i < N_SCAN; ++i)
        {
          ...
          for (int j = 0; j < 6; ++j)
          {
            ...
            if (sp >= ep){
              ROS_INFO("impossible");
              ROS_INFO("sp: %d, ep: %d, start: %d, end: %d", sp, ep, seg_info->startRingIndex[i], seg_info->endRingIndex[i]);
              continue;
            }
            TicToc t_tmp;
        ...
        ROS_INFO("less_sharp size: %d, less_flat size: %d", less_sharp->size(), less_flat->size());
        ...
        else
        {
          ...
          t_total += t_opt.toc();
          ROS_INFO("opt time until now: %.3fms", t_total);
  ...
  void adjustDistortion(PointCloudT::Ptr cloud, double scan_time)
  {
    ...
    if (ori_diff <= 0)
    {
      ROS_WARN("ori_diff <= 0, %d", ori_diff);
