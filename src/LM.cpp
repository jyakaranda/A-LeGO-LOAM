...
public:
    ...
    while (ros::ok())
    {
          ...
          std::abs(time_laser_surf_ - time_laser_corner_) < 0.005 && std::abs(time_laser_surf_ - time_laser_outlier_) < 0.005 && std::abs(time_laser_surf_ - time_laser_odom_) < 0.005)
      {
        ...
        if (frame_cnt % 2 == 0)
        {
          ...
          ROS_INFO("transformAssociateToMap: %.3fms", t_tatm.toc());
          ROS_INFO("extractSurroundingKeyFrames");
          TicToc t_eskf;
          extractSurroundingKeyFrames();
          ROS_INFO("extractSurroundingKeyFrames: %.3fms", t_eskf.toc());
          ROS_INFO("downsampleCurrentScan");
          TicToc t_dcs;
          downsampleCurrentScan();
          ROS_INFO("downsampleCurrentScan: %.3fms", t_dcs.toc());
          ROS_INFO("scan2MapOptimization");
          TicToc t_s2mo;
  ...
  void extractSurroundingKeyFrames()
  {
    ...
    ROS_INFO("before ds, surf points: %d, corner points: %d, time: %.3fms", surf_from_map_->size(), corner_from_map_->size());
    ROS_INFO("surf points: %d, corner points: %d, time: %.3fms", surf_from_map_ds_->size(), corner_from_map_ds_->size(), t_data.toc());
    ROS_INFO("downsize time: %.3fms", t_ds.toc());
  }
  ...
  void scan2MapOptimization()
  {
    ...
    ROS_INFO("build kdtree time: %.3fms", t_tree.toc());
    ...
    for (int iter_cnt = 0; iter_cnt < 2; ++iter_cnt)
    {
      ...
      ROS_INFO("mapping data assosiation time %.3fms", t_data.toc());
      ROS_INFO("corner_correspondance: %d, surf_correspondance: %d", corner_correspondace, surf_correnspondance);
      ...
      ROS_INFO("mapping solver time %.3fms", t_solver.toc());
  ...
  void loopClosureThread()
  {
    ...
    while (ros::ok())
    {
      ...
      TicToc t_plc;
      performLoopClosure();
      ROS_INFO("performLoopClosure: %.3fms", t_plc.toc());
      duration.sleep();
    }
  }
